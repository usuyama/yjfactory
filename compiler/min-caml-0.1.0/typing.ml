...
let rec deref_typ = function (* 型変数を中身でおきかえる関数 (caml2html: typing_deref) *)
  ...
  | Type.Var({ contents = Some(t) } as r) ->
	...
	t'
  | t -> t
let rec deref_id_typ (x, t, esc) = (x, deref_typ t, esc)
let rec deref_term = function
  ...
  | F2I(e) -> F2I(deref_term e)
  | I2F(e) -> I2F(deref_term e)
  | Sendc(e) -> Sendc(deref_term e)
  ...
  | Tuple(es, esc) -> Tuple(List.map deref_term es, esc)
  | LetTuple(xts, e1, e2) -> LetTuple(List.map deref_id_typ xts, deref_term e1, deref_term e2)
  | Array(e1, e2, esc) -> Array(deref_term e1, deref_term e2, esc)
...
let rec g env e = (* 型推論ルーチン (caml2html: typing_g) *)
  try
	...
	unify t2 t3;
	t2
    | Let((x, t, _), e1, e2) -> (* letの型推論 (caml2html: typing_let) *)
	unify t (g env e1);
	g (M.add x t env) e2
	...
	let t = Type.gentyp () in
	  ...
	  t
    | LetRec({ name = (x, t, _); args = yts; body = e1}, e2) -> (* let recの型推論 (caml2html: typing_letrec) *)
	let env = M.add x t env in
	  unify t (Type.Fun(List.map get_typ yts, g (M.add_list (List.map id_type_pair yts) env) e1));
	  g env e2
    ...
    | LetTuple(xts, e1, e2) ->
	unify (Type.Tuple(List.map get_typ xts)) (g env e1); (* とりあえずEscape.Neverにする *)
	g (M.add_list (List.map id_type_pair xts) env) e2
