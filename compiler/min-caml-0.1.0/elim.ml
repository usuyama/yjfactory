...
let rec effect = function (* 副作用の有無 (caml2html: elim_effect) *)
  ...
  | App _ | Put _ | ExtFunApp _ | Sendc _ -> true
  | _ -> false
...
let rec f = function (* 不要定義削除ルーチン本体 (caml2html: elim_f) *)
  ...
  | Let((x, t), e1, e2) -> (* letの場合 (caml2html: elim_let) *)
	...
	if effect e1' || S.mem x (fv e2') then Let((x, t), e1', e2') else
	  (Printf.printf "!!Let : liminating variable %s@.\n" x;
	   e2')
  | LetRec({ name = (x, t, esc); args = yts; body = e1 }, e2) -> (* let recの場合 (caml2html: elim_letrec) *)
      let e2' = f e2 in
	if S.mem x (fv e2') then
	  LetRec({ name = (x, t, esc); args = yts; body = f e1 }, e2')
	else
	  (Printf.printf "!!LetRec :: eliminating function %s@.\n" x;
	   e2')
  | LetTuple(xts, y, e) ->
	...
	if List.exists (fun x -> S.mem x live) xs then LetTuple(xts, y, e') else
	  (Printf.printf "!!LetTuple : eliminating variables %s@.\n" (Id.pp_list xs);
	   e')
  | e -> e
